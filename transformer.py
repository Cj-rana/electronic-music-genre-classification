...
def extract_features(file_path, sr=22050, n_mels=64, duration=5):
    try:
...
def create_transformer_model(input_shape, num_classes, num_heads=2, ff_dim=64, num_layers=2):
...
def classify_track(model, file_path, max_len, genres):
    ...
    if features is not None:
        features_padded = np.pad(features, ((0, 0), (0, max_len - features.shape[1])), mode='constant')
        features_padded = features_padded.transpose((1, 0))  # Reshape to (timesteps, features)
        features_padded = features_padded[np.newaxis, ...]  # Add batch dimension
...
if len(X_padded) == 0 or len(y) == 0:
    ...
    # Reshape for transformer input (batch_size, timesteps, features)
    X_padded = X_padded.transpose((0, 2, 1))  # Shape should be (batch_size, timesteps, features)
    ...
    for train, test in skf.split(X_padded, np.argmax(y, axis=1)):
        # Create model
        model = create_transformer_model((X_padded.shape[1], X_padded.shape[2]), len(genres))
        ...
        # Fit the model
        model.fit(X_padded[train], y[train], epochs=10, batch_size=64, verbose=1,
