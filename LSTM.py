...
# Define the LSTM model creation function with regularization
...
if len(X_padded) == 0 or len(y) == 0:
    ...
    for train, test in skf.split(X_padded, np.argmax(y, axis=1)):
        # Create model
        model = create_lstm_model((X_padded.shape[1], X_padded.shape[2]), len(genres))
        ...
        # Fit the model
        model.fit(X_padded[train], y[train], epochs=20, batch_size=64, verbose=1,  # Adjusted epochs
